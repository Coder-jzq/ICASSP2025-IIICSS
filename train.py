...
def train(rank,rank2, args, configs, batch_size, num_gpus):
    ...
    group_size = 2  # Set this larger than 1 to enable sorting in Dataset
        ...
        for batchs in loader:
            ...
            for batch in batchs:
                ...
                with amp.autocast(args.use_amp):
                    # Forward
                    output, interaction_output = model(*(batch[2:]), step=step)
                    batch[9:11], output = output[-2:], output[:-2] # Update pitch and energy level

                    # Cal Loss
                    losses = Loss(batch, output, interaction_output, step=step)
                    total_loss = losses[0]
                ...
                if rank == rank2:
                    if step % log_step == 0:
                        ...
                        message2 = "Total Loss: {:.4f}, Mel Loss: {:.4f}, Mel PostNet Loss: {:.4f}, Pitch Loss: {:.4f}, Energy Loss: {:.4f}, Duration Loss: {:.4f}, CTC Loss: {:.4f}, Binarization Loss: {:.4f}, cross_contrastive_loss_enhance_style: {:.4f}, cross_contrastive_loss_enhance_linguistic: {:.4f}, intra_contrastive_loss_enhance_style: {:.4f}, intra_contrastive_loss_enhance_linguistic: {:.4f}".format(
                            *losses_
                        )
...
if __name__ == "__main__":
    ...
    if num_gpus > 1:
        ...
        train(1, 1, args, configs, batch_size, num_gpus)
