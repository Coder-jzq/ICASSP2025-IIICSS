...
class Dataset(Dataset):
    def __init__(
        ...
        # audo 特征 大小
        self.audio_emb_size = model_config["history_encoder"]["audio_emb_size"]
        ...
        if self.history_type != "none":
            ...
            if self.history_type == "Guo":
                ...
                for i, h_basename in enumerate(history_basenames):
                    ...
                    h_audio_emb_path = os.path.join(
                        self.preprocessed_path,
                        "waviemocap_emb",
                        "{}-waviemocap_emb-{}.npy".format(h_speaker, h_basename),
                    )
                    h_audio_emb = np.load(h_audio_emb_path)
    ...
    def pad_history(self, 
        ...
        for _ in range(pad_size):
            ...
            history_audio_emb.insert(0,np.zeros(self.audio_emb_size, dtype=np.float32)) if history_audio_emb is not None else None
    ...
    def reprocess(self, data, idxs):
        ...
        if self.history_type != "none":
            if self.history_type == "Guo":
                ...
                audio_embs = [data[idx]["history"]["audio_emb"] for idx in idxs]
                history_audio_embs = [data[idx]["history"]["history_audio_emb"] for idx in idxs]
                ...
                history_audio_embs = np.array(history_audio_embs)
...
class TextDataset(Dataset):
    ...
    def pad_history(self, 
        ...
        for _ in range(pad_size):
            ...
            history_audio_emb.insert(0, np.zeros(self.audio_emb_size,
    ...
    def collate_fn(self, data):
        ...
        if self.history_type != "none":
            if self.history_type == "Guo":
                ...
                history_audio_embs = [d[6]["history_audio_emb"] for d in data]
