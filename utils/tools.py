    ...
    if len(data) == 16:
        ...
        if history_info is not None:
            if len(history_info) == 6: # "Guo"
                ...
                audio_embs = torch.from_numpy(audio_embs).float().to(device)
                history_audio_embs = torch.from_numpy(history_audio_embs).float().to(device)
...
def log(
    ...
    if losses is not None:
        ...
        logger.add_scalar("Loss/cross_contrastive_loss_enhance_style, ", losses[8], step)
        logger.add_scalar("Loss/cross_contrastive_loss_enhance_linguistic, ", losses[9], step)
        logger.add_scalar("Loss/intra_contrastive_loss_enhance_style, ", losses[10], step)
        logger.add_scalar("Loss/intra_contrastive_loss_enhance_linguistic, ", losses[11], step)
...
def synth_samples_reconstructed(targets, predictions, vocoder, model_config, preprocess_config, path, args):
    ...
    mel_GT = targets[6].transpose(1, 2)
    lengths = targets[7] * preprocess_config["preprocessing"]["stft"]["hop_length"]
    wav_predictions = vocoder_infer(
        mel_GT, vocoder, model_config, preprocess_config, lengths=lengths
    )
