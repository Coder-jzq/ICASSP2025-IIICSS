...
class TextEncoder(nn.Module):
    ...
    def __init__(self, config):
        ...
        d_word_vec = config["conformer"]["encoder_hidden"]
        n_layers = config["conformer"]["encoder_layer"]
        n_head = config["conformer"]["encoder_head"]
        d_model = config["conformer"]["encoder_hidden"]
        f_factor = config["conformer"]["feed_forward_expansion_factor"]
        c_factor = config["conformer"]["conv_expansion_factor"]
        kernel_size = config["conformer"]["conv_kernel_size"]
        half_step_residual = config["conformer"]["half_step_residual"]
        dropout = config["conformer"]["encoder_dropout"]
        ...
        self.layer_stack = nn.ModuleList(
            [
                ConformerBlock(
                    ...
                    feed_forward_expansion_factor = f_factor,
                    conv_expansion_factor = c_factor,
...
class Decoder(nn.Module):
    ...
    def __init__(self, config):
        ...
        n_layers = config["conformer"]["decoder_layer"]
        ...
        kernel_size = config["conformer"]["conv_kernel_size"]
        half_step_residual = config["conformer"]["half_step_residual"]
        dropout = config["conformer"]["decoder_dropout"]
    ...
    def forward(self, enc_seq, mask):
        ...
        for dec_layer in self.layer_stack:
            dec_output = dec_layer(dec_output, mask=mask)
...
class ConformerBlock(nn.Module):
    ...
    Args:
        ...
        position_enc: Tensor of positional encoding
...
class MultiHeadedSelfAttentionModule(nn.Module):
    ...
    Args:
        d_model (int): The dimension of models
...
class RelativeMultiHeadAttention(nn.Module):
    ...
    def __init__(
        ...
        self.query_proj = LinearNorm(d_model, d_model)
        self.key_proj = LinearNorm(d_model, d_model)
        self.value_proj = LinearNorm(d_model, d_model)
        self.pos_proj = LinearNorm(d_model, d_model, bias=False)
