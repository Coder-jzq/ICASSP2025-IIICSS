...
class VarianceAdaptor(nn.Module):
    ...
    def forward(
        ...
        attn_prior=None,
        cross_linguistic_enhancement_hidden=None,
        cross_style_memory_enhancement_hidden=None,
        intra_linguistic_enhancement_hidden=None,
        intra_style_memory_enhancement_hidden=None,
        p_control=1.0,
        e_control=1.0,
        d_control=1.0,
        step=None,
        ...
        # ---------------------------------------- (cross & intra) --------------------------------------------
        if cross_linguistic_enhancement_hidden is not None:
            x = x + cross_linguistic_enhancement_hidden.unsqueeze(1).expand(
                -1, text.shape[1], -1
            )

        if cross_style_memory_enhancement_hidden is not None:
            x = x + cross_style_memory_enhancement_hidden.unsqueeze(1).expand(
                -1, text.shape[1], -1
            )

        if intra_linguistic_enhancement_hidden is not None:
            x = x + intra_linguistic_enhancement_hidden.unsqueeze(1).expand(
                -1, text.shape[1], -1
            )

        if intra_style_memory_enhancement_hidden is not None:
            x = x + intra_style_memory_enhancement_hidden.unsqueeze(1).expand(
                -1, text.shape[1], -1
            )
...
class StyleHisAttn(nn.Module):
    def __init__(self, d_enc, temperature):
        super(StyleHisAttn, self).__init__()
        self.linear = nn.Linear(d_enc, 1)  #(768, 1)
...
class LinguisticHisAttn(nn.Module):
    def __init__(self, d_enc, temperature):
        super(LinguisticHisAttn, self).__init__()
        self.linear = nn.Linear(d_enc, 1)  #(516, 1)
        self.softmax = nn.Softmax(dim=2)
        self.temperature = temperature
    def forward(self, Q, K, V):
        resnet = Q
...
class StyleEncoder(nn.Module):
    def __init__(self, preprocess_config, model_config):
        ...
        self.gru_in_features = model_config["style_enc"]["gru_in_features"]
        self.gru_out_features = model_config["style_enc"]["gru_out_features"]
        self.gru_num_layers = model_config["style_enc"]["gru_num_layers"]
        self.dropout = model_config["style_enc"]["dropout"]
        self.layer_features = model_config["style_enc"]["layer_features"]

        self.gru = nn.GRU(self.gru_in_features, self.gru_out_features, self.gru_num_layers, batch_first=True, bidirectional=True)

        self.layer1 = nn.Linear(self.gru_out_features * 2, self.gru_out_features)
        self.layer2 = nn.Linear(self.gru_out_features, self.layer_features)
        ...
        self.speaker_embedding = nn.Embedding(
            n_speaker,
            self.gru_in_features,
        )
        self.style_spk_layer = nn.Linear(self.gru_in_features * 2, self.gru_in_features)

    def forward(self, cur_features, past_features, history_speaker, speaker):
        cur_features = cur_features.unsqueeze(dim=1)
        input_features = torch.cat([past_features, cur_features], dim=1)

        history_speaker = torch.cat([history_speaker, speaker.unsqueeze(1)], dim=1)
        history_speaker_emb = self.speaker_embedding(history_speaker)

        input_features = torch.cat([input_features, history_speaker_emb], dim=-1)
        input_features = self.style_spk_layer(input_features)

        gru_output = self.gru(input_features)

        output = self.dropout_layer(
            F.relu(self.layer1(gru_output[0]))
        )
        output = self.dropout_layer(
            self.layer2(output)
        )
        return output
class StyleMemoryEnhancementEncoder(nn.Module):

    def __init__(self, preprocess_config, model_config):
        super(StyleMemoryEnhancementEncoder, self).__init__()
        self.gru_in_features = model_config["style_enc"]["gru_in_features"]
        self.gru_out_features = model_config["style_enc"]["gru_out_features"]
        self.gru_num_layers = model_config["style_enc"]["gru_num_layers"]
        self.dropout = model_config["style_enc"]["dropout"]
        ...
        self.style_his_attn = StyleHisAttn(self.layer_features, np.power(self.layer_features, 0.5))
    ...
    def forward(self, past_features, history_speaker):

        history_speaker_emb = self.speaker_embedding(history_speaker)

        past_features = torch.cat([past_features, history_speaker_emb], dim=-1)
        past_features = self.style_spk_layer(past_features)

        gru_output = self.gru(past_features)
        ...
        # S1， S1-2， S1-3， ...， S1-（C-1）
        output_attn = []
        output_attn.append(output_K.unsqueeze(dim=1))

        # output.shape[1] - 1
        for idx in range(1, output.shape[1]):
            output_Q = output[:, idx, :]

            output_K = self.style_his_attn(
                output_Q,
                output_K,
                output_K
            )
        ...
        output_attn = torch.cat(output_attn, dim=1)

        return output_attn
class LinguisticEncoder(nn.Module):
    def __init__(self, preprocess_config, model_config):
        super(LinguisticEncoder, self).__init__()
        self.gru_in_features = model_config["linguistic_enc"]["gru_in_features"]
        self.gru_out_features = model_config["linguistic_enc"]["gru_out_features"]
        self.gru_num_layers = model_config["linguistic_enc"]["gru_num_layers"]
        self.dropout = model_config["linguistic_enc"]["dropout"]
        self.layer_features = model_config["linguistic_enc"]["layer_features"]

        self.gru = nn.GRU(self.gru_in_features, self.gru_out_features, self.gru_num_layers, batch_first=True,
                          bidirectional=True)
    ...
    def forward(self, cur_features, past_features, history_speaker, speaker):
        ...
        return output
class LinguisticMemoryEnhancementEncoder(nn.Module):
    def __init__(self, preprocess_config, model_config):
        super(LinguisticMemoryEnhancementEncoder, self).__init__()
        self.gru_in_features = model_config["linguistic_enc"]["gru_in_features"]
        self.gru_out_features = model_config["linguistic_enc"]["gru_out_features"]
        self.gru_num_layers = model_config["linguistic_enc"]["gru_num_layers"]
        self.dropout = model_config["linguistic_enc"]["dropout"]
        ...
        self.linguistic_his_attn = LinguisticHisAttn(self.layer_features, np.power(self.layer_features, 0.5))
    ...
    def forward(self, past_features, history_speaker):
        ...
        for idx in range(1, output.shape[1]):
            ...
            output_K = self.linguistic_his_attn(
                output_Q,
                output_K,
                output_K
            )
